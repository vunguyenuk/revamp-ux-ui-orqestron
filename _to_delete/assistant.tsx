...
export default function TransactionWorkspace({
  ...
  const reply = (prompt: string) => {
    ...
    if (/(create|start|open|new|set up)/.test(lower) && /(transaction|deal|file|escrow)/.test(lower)) {
      ...
      const buyers = couple ? "Married couple · 2 buyers" : "1 buyer";
      setDealName(address);
      setDeals((current) => [
        ...current,
      ...
      fill([
        ...
        ...(couple
          ? [
              { key: "buyer1", value: "Buyer 1 — name needed", source: "review" as FieldSource },
              { key: "buyer2", value: "Buyer 2 — name needed", source: "review" as FieldSource },
            ]
          : []),
      ]);
      ...
      push({
        role: "agent",
        text: `Created and opened. Address and buyers are on the transaction file, so no form will ask for them again. ${firstQuestion}`,
      });
  ...
  const suggestions = (): string[] => {
    if (stage === "idle")
      return ["Create a new transaction for 123 Fifth Street, buyers are a married couple"];
    if (stage === "created")
      return [
        "Add the RPA and fill it in for me",
        "Buyers are David and Mai Nguyen",
  ...
  const toggleVoice = () => {
    if (recording) {
      ...
      setRecording(false);
      setVoiceHint(text.trim() ? "Stopped — press send when it looks right." : "");
      return;
    }
        ...
        <div className={a.split}>
          <div className={a.formPane}>
            <header className={a.formHead}>
              <span>
                <b>RPA</b> · California Residential Purchase Agreement
              </span>
              <span className={a.formMeta}>
                <small>
                  {sectionsDone} of {sections.length} sections complete · {filledCount}/
                  {totalFields} fields
                </small>
                      ...
                      <div key={field.key} className={a[field.source]}>
                        ...
                        <dd>
                          {editingField === field.key ? (
                            <input
                              autoFocus
                              className={a.fieldInput}
                              value={draft}
                              aria-label={`Edit ${field.label}`}
                              onChange={(event) => setDraft(event.target.value)}
                              onBlur={() => saveFieldEdit(field.key, field.label)}
                              onKeyDown={(event) => {
                                if (event.key === "Enter") {
                                  event.preventDefault();
